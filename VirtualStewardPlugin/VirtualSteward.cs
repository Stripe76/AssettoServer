...
public class VirtualStewardPlugin : CriticalBackgroundService, IAssettoServerAutostart
{
    ...
    public VirtualStewardPlugin( ACUdpServer udpServer,SessionManager sessionManager,VirtualStewardConfiguration configuration,ACServerConfiguration serverConfiguration,EntryCarManager entryCarManager,IHostApplicationLifetime applicationLifetime ) : base( applicationLifetime )
    {
    ...
    private void SendUpdate( EntryCar fromCar )
    {
        for( int j = 0; j < _entryCarManager.EntryCars.Length; j++ )
        {
            ...
            if( toCar == fromCar
                ...
                || !fromCar.GetPositionUpdateForCar( toCar,out var update ) )
                continue;

            SendPacketUdp( toClient,in update );
        }

    }

    private void SendPacketUdp<TPacket>( ACTcpClient client, in TPacket packet ) where TPacket : IOutgoingNetworkPacket
    {
        //if( UdpEndpoint == null )
          //  return;
        ...
        try
        {
            ...
            //Logger.Debug( "Sending UDP packet '{PacketId:X}'",packet.ToString( ) );

            //_udpServer.Send( UdpEndpoint,buffer,0,bytesWritten );
        }
        catch( Exception ex )
        {
        }
    ...
    {
        //_ = LoadReplayAsync( stoppingToken );
        //_ = UpdateAsync( stoppingToken );

        LoadReplay( "replay.acreplay" );
